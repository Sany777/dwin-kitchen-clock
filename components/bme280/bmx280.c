 ...
 */
static esp_err_t
bmx280_read(bmx280_t *bmx280, uint8_t addr, uint8_t *dout, size_t size)
{
    esp_err_t err;
...
static esp_err_t
bmx280_write(bmx280_t *bmx280, uint8_t addr, const uint8_t *din, size_t size)
{
    esp_err_t err;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    if (cmd) {
...
static esp_err_t
bmx280_probe_address(bmx280_t *bmx280)
{
...
static esp_err_t
bmx280_calibrate(bmx280_t *bmx280)
{
...
bmx280_t *
bmx280_create(i2c_port_t port)
{
    static bmx280_t bmx280_static_inst;
    bmx280_t *bmx280 = &bmx280_static_inst;
    if (bmx280) {
...
esp_err_t
bmx280_configure(bmx280_t *bmx280, bmx280_config_t *cfg)
{
...
esp_err_t
bmx280_setMode(bmx280_t *bmx280, bmx280_mode_t mode)
{
    uint8_t ctrl_mes;
    esp_err_t err;
...
esp_err_t
bmx280_getMode(bmx280_t *bmx280, bmx280_mode_t *mode)
{
    uint8_t ctrl_mes;
    esp_err_t err;
...
bool
bmx280_isSampling(bmx280_t *bmx280)
{
    uint8_t status;
...
int32_t
BME280_compensate_T_int32(bmx280_t *bmx280, int32_t adc_T)
{
    int32_t var1, var2, T;
...
uint32_t
BME280_compensate_P_int64(bmx280_t *bmx280, int32_t adc_P)
{
    int64_t var1, var2, p;
...
uint32_t
bme280_compensate_H_int32(bmx280_t *bmx280, int32_t adc_H)
{
    int32_t v_x1_u32r;
...
esp_err_t
bmx280_readTemp(bmx280_t *bmx280, float *temperature)
{
    int32_t t;
...
esp_err_t
bmx280_readout(bmx280_t *bmx280, int32_t *temperature, uint32_t *pressure, uint32_t *humidity)
{
    if (bmx280 == NULL)
        return ESP_ERR_INVALID_ARG;
    if (!bmx280_validate(bmx280))
...
#endif
    {
        if (humidity) {
            ...
            *humidity = bme280_compensate_H_int32(bmx280, (buffer[0] << 8) | buffer[0]);
        }
    }
#if CONFIG_BMX280_EXPECT_DETECT
    else if (humidity)
