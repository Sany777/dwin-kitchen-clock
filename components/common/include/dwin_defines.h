...
#define KEY_IS_SET_TASK(key_from_dwin) ((key_from_dwin) >= START_SCREEN_TASK && (key_from_dwin) < END_KEY_SCREEN_TASK)

#define GET_SCREEN_TASK(key_from_dwin) ((key_from_dwin) - START_SCREEN_TASK)

/*NOTIFICATIONS*/
#define GET_NOTIF_HOUR(_number_notif, _day_week)                                                                       \
    (notification_DATA[(_day_week) * (NOTIF_PER_DAY) * 2 + (_number_notif) * 2])

#define VALUE_NOTIF_HOUR(_number_notif, _day_week) (GET_NOTIF_HOUR(_number_notif, _day_week) % 100)

#define VALUE_NOTIF_MIN(_number_notif, _day_week)                                                                      \
    (notification_DATA[(_day_week) * (NOTIF_PER_DAY) * 2 + (_number_notif) * 2 + 1])

#define IS_NOTIF_ACTIVE(_notif, _day) (GET_NOTIF_HOUR(_notif, _day) < 99)

#define IS_DAY_ACTIVE(_day)                                                                                            \
    (IS_NOTIF_ACTIVE(0, _day) || IS_NOTIF_ACTIVE(1, _day) || IS_NOTIF_ACTIVE(2, _day) || IS_NOTIF_ACTIVE(3, _day))
...
/* CHECK INPUT KEY*/
#define KEY_IS_CHAR(key_from_dwin)                                                                                     \
    (((key_from_dwin) >= 'A' && (key_from_dwin) <= 'Z') || ((key_from_dwin) >= 'a' && (key_from_dwin) <= 'z'))

#define KEY_IS_NUMBER(key_from_dwin) ((key_from_dwin) >= '0' && (key_from_dwin) <= '9')
#define KEY_IS_AREA_SCAN_SSID(key_from_dwin)                                                                           \
    ((key_from_dwin) >= KEY_START_AREA && (key_from_dwin) < KEY_START_AREA + MAX_SCAN_LIST_SIZE)

#define KEY_IS_AREA_CLOCK(key_from_dwin)                                                                               \
    ((key_from_dwin) >= KEY_START_AREA && (key_from_dwin) < KEY_START_AREA + END_AREA_CLOCK)

#define KEY_IS_AREA_TIMERS(key_from_dwin)                                                                              \
    ((key_from_dwin) >= KEY_START_AREA && (key_from_dwin) < KEY_START_AREA + END_AREA_TIMERS)

#define KEY_IS_AREA_STOP_TIMER(key_from_dwin)                                                                          \
    ((key_from_dwin) >= KEY_START_AREA && (key_from_dwin) < KEY_START_AREA + END_AREA_STOP_TIMERS)

#define KEY_IS_AREA_NOTIF(key_from_dwin)                                                                               \
    ((key_from_dwin) >= KEY_START_AREA && (key_from_dwin) < KEY_START_AREA + END_AREA_NOTIFICATIONS)

#define KEY_IS_AREA_CUSTOM(key_from_dwin)                                                                              \
    ((key_from_dwin) >= KEY_START_AREA && (key_from_dwin) < KEY_START_AREA + END_AREA_COLORS)

#define KEY_IS_AREA_SETTING(key_from_dwin)                                                                             \
    ((key_from_dwin) >= KEY_START_AREA && (key_from_dwin) < KEY_START_AREA + END_AREA_SETTINGS)

#define KEY_IS_SYMBOL(key_from_dwin) ((key_from_dwin) >= '!' && (key_from_dwin) <= '~')

#define KEY_IS_AREA_TOGGLE(key_from_dwin)                                                                              \
    ((key_from_dwin) >= KEY_TOGGLE_1 && (key_from_dwin) < (KEY_TOGGLE_1 + MAX_TOGGLE_UNIT_ON_SCREEN))

#define KEY_IS_AREA_TOGGLE_DAY(key_from_dwin)                                                                          \
    ((key_from_dwin) >= KEY_TOGGLE_DAY_1 && (key_from_dwin) < (KEY_TOGGLE_DAY_1 + SIZE_WEEK))

#define KEY_IS_DAY(key_from_dwin) ((key_from_dwin) >= KEY_DAY_1 && (key_from_dwin) < KEY_DAY_1 + SIZE_WEEK)
