...
static char server_buf[SCRATCH_SIZE];
...
static esp_err_t handler_save_pic(httpd_req_t *req)
{
    ...
    dwin_save_picture(pos_pic);
    char buf_resp[LEN_RESPONSE_SAVE+10];
...
{
    char notif_send[LEN_DATA_SEND_NOTIF];
    dwin_data_t *main_data = (dwin_data_t *)req->user_ctx;
...
static esp_err_t handler_set_flag(httpd_req_t *req)
{
    ...
    dwin_memory_write(NULL, DATA_FLAGS);
...
static esp_err_t set_notif_handler(httpd_req_t *req)
{
    ...
    char * const loc_buf = server_buf;
    dwin_data_t * main_data = (dwin_data_t *)req->user_ctx;
    const int received = httpd_req_recv(req, loc_buf, total_len);
    if (received != total_len) {
        DWIN_RESP_ERR(req, "Data not read", err);
    }
    for(size_t  i=0,num_notif = 0,dayi=0,val=0; i<total_len; dayi++){
        ...
        val = GET_NUMBER(loc_buf[i])*10 + GET_NUMBER(loc_buf[i+1]);
        if(!IS_HOUR(val)){
           DWIN_RESP_ERR(req, "Value hour is wrong", err); 
        }
        SET_NOTIF_HOUR(num_notif, dayi, val);
        val = GET_NUMBER(loc_buf[i+2])*10 + GET_NUMBER(loc_buf[i+3]);
        if(!IS_MIN_OR_SEC(val)){
           DWIN_RESP_ERR(req, "Value minute is wrong", err); 
        }
