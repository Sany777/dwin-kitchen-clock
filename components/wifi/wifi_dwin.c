...
void
set_wifi(dwin_data_t *main_data, uint8_t action)
{
    ...
    case START_ESPNOW: {
        if (!init_espnow) {
            ...
            if (!rx_espnow)
                xTaskCreate(espnow_task_rx, "espnow_task_rx", 4096, main_data, 5, &rx_espnow);
            if (!tx_espnow)
                xTaskCreate(espnow_task_tx, "espnow_task_tx", 4096, main_data, 5, &tx_espnow);
            if (!rx_espnow || !tx_espnow)
                return;
    ...
    case STOP_ESPNOW: {
        if (init_espnow) {
            ...
            while (xQueueReceive(espnow_rx_queue, &data_rx, 0) == pdTRUE) {
            }
            ...
            if (espnow_tx_queue) {
                vQueueDelete(espnow_tx_queue);
                espnow_tx_queue = NULL;
            }
            esp_now_deinit();
        ...
        if (mode != WIFI_MODE_AP) {
            if (mode != WIFI_MODE_NULL) {
                if (xEventGroup & BIT_ESPNOW_RUN) {
                    dwin_timer_set_new_command(STOP_ESPNOW);
                }
                esp_wifi_stop();
                vTaskDelay(400);
            }
    ...
    case START_STA: {
        ...
        if (xEventGroup & BIT_ESPNOW_ALLOW && !(xEventGroup & BIT_ESPNOW_RUN)) {
            dwin_timer_set_periodic_event(INIT_ESPNOW, 30, ONLY_ONCE);
        }
        break;
    }
    case CLOSE_CUR_CON: {
...
ap_handler(void *main_data, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_id == WIFI_EVENT_AP_STOP) {
        ...
        dwin_timer_set_new_command(STATION_JOINE);
...
void
wifi_sta_handler(void *data, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (!(xEventGroup & BIT_DENIED_STA)) {
        if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
            ...
            if (usd_Sale == DWIN_NO_DATA)
                dwin_timer_set_periodic_event(UPDATE_CURRENCY, 10, RELOAD_COUNT);
...
void
set_sntp(dwin_data_t *main_data, uint8_t action)
{
    if (action == INIT_SNTP) {
        if (!esp_sntp_enabled()) {
