...
void init_uart()
{
    ...
    uart_set_pin(UART_DWIN, TXD_PIN, RXD_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
...
void task_uart_event(void *pv)
{
    ...
    static uint8_t buf_RX[UART_BUF_SIZE];
    uart_event_t event;
for(;;) {
                ...
                if (heder_ok) {
                        ...
                        && (buf_RX[byte_rx_count] == 0x3C))
                    {
                        if(buf_RX[INDEX_IDENTIF_DATA_IN_RX] == TOUCH_CODE) {
                            xQueueSend(direct_queue, &buf_RX[INDEX_START_DATA_IN_RX], 300);
