...
void screen_handler_device(dwin_data_t *main_data, uint8_t command,
                          char symbol) {
  if (command == KEY_INIT) {
    dwin_set_picture(INFO_PIC);
    vTaskDelay(DELAY_CHANGE_PIC);
...
void screen_handler_wifi_scan(dwin_data_t *main_data, uint8_t command,
                             char symbol) {
  static wifi_ap_record_t ap_info[MAX_SCAN_LIST_SIZE];
  ...
  if (command == KEY_INIT) {
    ...
    if (area_SCREEN == select_area) {
      ...
      if (next_page)
        ind += MAX_SSID_PEER_SCREEN;
      strncpy(name_SSID, (const char *)ap_info[ind].ssid, MAX_STR_LEN);
      show_data_send_message("The SSID has been set");
  ...
  dwin_set_picture(SEARCH_PIC);
  show_screen(next_page ? ap_count * -1 : ap_count, &ap_info, sizeof(ap_info));
}
...
void screen_handler_ap(dwin_data_t *main_data, uint8_t command, char symbol) {
  if (command == KEY_INIT) {
    ...
    if (start_server(main_data) == ESP_OK) {
      ...
      show_screen(UPDATE_DATA_COMPLETE, NULL, 0);
      dwin_timer_set_new_command(INIT_AP);
    } else {
      stop_server();
      dwin_timer_set_periodic_event(MAIN_SCREEN, 1, ONLY_ONCE);
    }
    ...
    dwin_timer_remove_periodic_event(MAIN_SCREEN);
...
void screen_handler_settings(dwin_data_t *main_data, uint8_t command,
  ...
  if (command == KEY_INIT) {
    dwin_set_picture(SETTING_LOW_LETTER_PIC);
    vTaskDelay(DELAY_CHANGE_PIC);
    dwin_timer_set_periodic_event(START_STA, 2, ONLY_ONCE);
    ...
    if (need_write) {
      dwin_memory_write(main_data, DATA_API);
      dwin_memory_write(main_data, DATA_CITY);
    ...
    switch (area_SCREEN) {
    case AREA_PASSWORD:
      selected_buf = pwd_WIFI;
      break;
    case AREA_CITY:
      selected_buf = name_CITY;
      break;
    case AREA_API:
      selected_buf = api_KEY;
      break;
    case AREA_SSID:
      selected_buf = name_SSID;
      break;
...
void screen_handler_main(dwin_data_t *main_data, uint8_t command, char symbol) {
  ...
  struct tm *cur_time = wifi_dwin_get_time_tm();
  switch (command) {
  case KEY_INIT: {
    details = false;
    menu_active = false;
    atempt = 0;
    dwin_timer_set_periodic_event(UPDATE_TIME_COMPLETE, DELAI_UPDATE_TIME_ON_SCREEN,
                       RELOAD_COUNT);
    dwin_timer_set_periodic_event(GET_WEATHER, DELAI_FIRST_UPDATE_WEATHER, ONLY_ONCE);
    dwin_timer_remove_periodic_event(MAIN_SCREEN);
    break;
  }
  case KEY_CLOSE: {
    dwin_timer_remove_periodic_event(GET_WEATHER);
    dwin_timer_remove_periodic_event(UPDATE_TIME_COMPLETE);
    dwin_timer_remove_periodic_event(KEY_MENU_SCREEN);
    return;
  }
  case UPDATE_WEATHER_COMPLETE: {
    if (weather_PIC == NO_WEATHER_PIC) {
      if (atempt < ATEMPT_GET_WEATHER) {
        atempt++;
        dwin_timer_set_periodic_event(GET_WEATHER,
                           DELAI_FIRST_UPDATE_WEATHER * atempt * atempt,
                           ONLY_ONCE);
      } else {
        dwin_timer_set_periodic_event(GET_WEATHER, DELAI_UPDATE_WEATHER_FAIL, ONLY_ONCE);
      }
    } else {
      if (atempt)
        atempt = 0;
      dwin_timer_set_periodic_event(GET_WEATHER, DELAI_UPDATE_WEATHER, ONLY_ONCE);
    }
    break;
  }
  case KEY_DETAILS_SCREEN: {
    if (details) {
      ...
      dwin_timer_set_periodic_event(UPDATE_TIME_COMPLETE, DELAI_UPDATE_TIME_ON_SCREEN,
                         RELOAD_COUNT);
      dwin_timer_remove_periodic_event(KEY_DETAILS_SCREEN);

    } else if (weather_PIC != NO_WEATHER_PIC) {
      details = true;
      dwin_timer_remove_periodic_event(UPDATE_TIME_COMPLETE);
      dwin_timer_set_periodic_event(KEY_DETAILS_SCREEN, 30, ONLY_ONCE);
    } else {
      dwin_timer_set_new_command(GET_WEATHER);
    }
    break;
  }
  case KEY_MENU_SCREEN: {
    if (menu_active) {
      ...
      menu_active = true;
      dwin_timer_remove_periodic_event(UPDATE_TIME_COMPLETE);
      dwin_timer_set_periodic_event(KEY_MENU_SCREEN, 30, ONLY_ONCE);
    }
    break;
  }
  ...
  if (details) {
    ...
    dwin_set_picture(MENU_PIC);
  } else {
    dwin_set_picture(weather_PIC);
    vTaskDelay(DELAY_CHANGE_PIC);
    show_screen(is_notify ? IS_NOTIFICATION : NORMAL_SCREEN, cur_time,
                sizeof(struct tm));
  }
}
...
void screen_handler_clock(dwin_data_t *main_data, uint8_t command, char symbol) {
  ...
  if (KEY_IS_AREA_CLOCK(command)) {
    ...
    case AREA_YEAR: {
      dwin_time->tm_year =
          dwin_time->tm_year > 10 ? val : dwin_time->tm_year * 10 + val;
      break;
    }
    case AREA_MONTH: {
      dwin_time->tm_mon = dwin_time->tm_mon == 1 && val < 3 ? 10 + val
                          : val > 0                         ? val
                                                            : 1;
      break;
    }
    ...
    dwin_memory_write_offset(offset);
    dwin_time->tm_year += 100;
    dwin_time->tm_mon--;
    wifi_dwin_set_timezone(offset);
    wifi_dwin_set_time_tm(dwin_time);
    dwin_time->tm_mon++;
    dwin_time->tm_year -= 100;
  } else if (command == KEY_SYNC) {
    ...
    if (GET_VALUE_TOGGLE(command) == 0) {
      ...
      dwin_memory_write(main_data, DATA_FLAGS);
    }
  ...
  if (command == KEY_INIT || command == UPDATE_TIME_COMPLETE ||
      command == KEY_ENTER) {
    if (!dwin_time) {
      static struct tm dwin_time_static;
      dwin_time = &dwin_time_static;
      if (!dwin_time)
        return;
...
void handle_state_screen(dwin_data_t *main_data, uint8_t command, char symbol) {
  ...
  if (command == KEY_SOUND_TOGGLE) {
    ...
    if (xEventGroup & BIT_ESPNOW_ALLOW) {
      ...
      dwin_timer_set_periodic_event(STOP_ESPNOW, 2, ONLY_ONCE);
    } else {
      xEventGroupSetBits(dwin_event_group, BIT_ESPNOW_ALLOW);
      dwin_timer_set_periodic_event(START_ESPNOW, 2, ONLY_ONCE);
    }
  } else if (command == KEY_SNTP_TOGGLE) {
    if (xEventGroup & BIT_SNTP_ALLOW) {
      xEventGroupClearBits(dwin_event_group, BIT_SNTP_ALLOW);
      dwin_timer_set_periodic_event(STOP_SNTP, 2, ONLY_ONCE);
    } else {
      xEventGroupSetBits(dwin_event_group, BIT_SNTP_ALLOW);
      dwin_timer_set_periodic_event(INIT_SNTP, 2, ONLY_ONCE);
    }
    ...
    if (xEventGroup & BIT_SECURITY) {
      ...
      pwd_WIFI[0] = 0;
      dwin_memory_write(main_data, DATA_PWD);
      show_data_send_message("Need set\n password WiFi!");
      dwin_set_picture(INFO_PIC);
    } else {
...
void handle_color_settings_screen(dwin_data_t *main_data, uint8_t command,
  ...
  if (KEY_IS_AREA_CUSTOM(command)) {
    ...
    dwin_memory_write(main_data, DATA_COLOR);
  }
  show_screen(UPDATE_DATA_COMPLETE, NULL, 0);
}
...
void screen_handler_notifications(dwin_data_t *main_data, uint8_t command,
  ...
  if (command == KEY_INIT) {
    struct tm *cut_time = wifi_dwin_get_time_tm();
    ...
    dwin_memory_write(main_data, DATA_NOTIF);
  } else if (KEY_IS_AREA_TOGGLE(command)) {
...
void screen_handler_timer(dwin_data_t *main_data, uint8_t command, char symbol) {
  ...
  if (command == KEY_CLOSE) {
    dwin_timer_remove_periodic_event(KEY_DECREMENT);
    dwin_print_end();
    vTaskDelay(200);
    dwin_clock_off();
    if (timer_run) {
  ...
  if (command == KEY_ENTER) {
    if (!timer_run) {
      ...
      dwin_clock_off();
      timer_run = true;
      dwin_timer_set_periodic_event(KEY_DECREMENT, 1, RELOAD_COUNT);
    }
  } else if (command == KEY_DECREMENT) {
    if (timer_SEC == 0 && timer_MIN == 0 && timer_HOUR == 0) {
      if (count_buzer) {
        dwin_sound_buzzer(LOUD_BUZZER);
        count_buzer--;
      } else {
        dwin_timer_remove_periodic_event(KEY_DECREMENT);
        dwin_timer_set_new_command(KEY_INIT);
        count_buzer = NUMBER_SIG_BUZ;
      }
      return;
    } else {
      dwin_set_picture(TIMER_RUN_PIC);
      timer_SEC--;
      if (timer_SEC < 0) {
        timer_SEC = 59;
        timer_MIN--;
    ...
    if (timer_run) {
      ...
      dwin_set_picture(TIMER_STOP_PIC);
      dwin_timer_remove_periodic_event(KEY_DECREMENT);
      timer_run = false;
    }
  } else if (command == KEY_INIT) {
  ...
  if (KEY_IS_AREA_TIMERS(command)) {
    ...
    if (new_area == area_SCREEN) {
      ...
      case AREA_TIMER_MIN: {
        timer_MIN += 5;
        if (timer_MIN % 5)
          timer_MIN = (timer_MIN / 5) * 5;
        if (!IS_MIN_OR_SEC(timer_MIN))
          timer_MIN = 0;
        break;
      }
      case AREA_TIMER_SEC: {
        timer_SEC += 10;
        if (timer_SEC % 10)
          timer_SEC = (timer_SEC / 10) * 10;
        if (!IS_MIN_OR_SEC(timer_SEC))
          timer_SEC = 0;
      }
      default:
        break;
