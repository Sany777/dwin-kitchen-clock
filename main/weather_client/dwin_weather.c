...
void get_weather_handler(void* args, esp_event_base_t base, int32_t key, void* event_data)
{
    ...
    xEventGroup = xEventGroupWaitBits(dwin_event_group, BIT_WIFI_STA, false, false, FIRST_WAIT_WIFI_BIT);
    if(!(xEventGroup&BIT_WIFI_STA)){
        start_sta();
        xEventGroup = xEventGroupWaitBits(dwin_event_group, BIT_WIFI_STA, false, false, SECOND_WAIT_WIFI_BIT);
    }
    if(xEventGroup & BIT_WIFI_STA){
